...
let rec aux number acc =
  ...
  |0 -> let x1,y1 = generate_coo acc x y in aux (number-1) ({width = 70; height = 50; x = x1; y =y1; texture = Graphic.texture_from_int 0}::acc)
  |1 -> let x1,y1 = generate_coo acc x y in aux (number-1) ({width = 70; height = 70; x = x1; y =y1; texture = Graphic.texture_from_int 1}::acc)
  |2 -> let x1,y1 = generate_coo acc x y in aux (number-1) ({width = 100; height = 100; x = x1; y =y1; texture = Graphic.texture_from_int 2}::acc)
