...
let generate_tree map n x y = 
  let rec aux number acc =
    if number = 0 then acc
      ...
      aux (number-1) ({inside = None; width = 5; height = 5; x = x1; y =y1; texture = Graphic.texture_tree_from_int (Random.int 15)}::acc)
  in aux n map
...
let generate_batiments n x y=
  let rec aux number acc =
    ...
    |0 -> let x1,y1 = generate_coo acc x y in aux (number-1) ({width = 100; height = 80; x = x1; y =y1; texture = Graphic.texture_batiment_from_int 0; inside = Some (Graphic.texture_crop_and_resize "./images/indoor.png" 0. 0. 224. 200. 1500 1000)}::acc)
    |1 -> let x1,y1 = generate_coo acc x y in aux (number-1) ({width = 107; height = 65; x = x1; y =y1; texture = Graphic.texture_batiment_from_int 1;  inside = Some (Graphic.texture_crop_and_resize "./images/indoor.png" 0. 0. 224. 200. 1500 1000)}::acc)
    |2 -> let x1,y1 = generate_coo acc x y in aux (number-1) ({width = 107; height = 65; x = x1; y =y1; texture = Graphic.texture_batiment_from_int 2;  inside = Some (Graphic.texture_crop_and_resize "./images/indoor.png" 0. 0. 224. 200. 1500 1000)}::acc)
...
let generate_road xchunk ychunk = 
    ...
    if n <= 0 then acc
      ...
      let bat : Types.batiments = {width=50; height=50; x=x1 ;y=y1 ; texture = text_road; inside = None} in 
